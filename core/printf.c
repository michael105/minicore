...
COMPILE ltodec itodec itoHEX atol
				...
				switch (fmt[a]){
					...
					case 'u':
						...
						if ( mod == 0 ){
							...
							b = b + uitodec( &buf[b],(convert_args?atoi(*args):(int)**args),(padding?padding:1),pad,sep);
							args++;
						...
						if ( mod == 1 ){
							...
							b = b + ultodec(&buf[b],(convert_args?atol(*args):(long)**args),(padding?padding:1),pad,sep);
					...
					case 'd':
						...
						if ( mod == 0 ){
							CHECKARG;
							MINI_TEST_OVERRUN(b+13);
							b = b + itodec(&buf[b],(convert_args?atoi(*args):(int)**args),padding,sep,pad);
							args++;
						...
						if ( mod == 1 ){
							CHECKARG;
							MINI_TEST_OVERRUN(b+27);
							b = b + ltodec(&buf[b],(convert_args?atoi(*args):(int)**args),(padding?padding:1),pad,sep);
							args++;
							//b = b + ltodec(va_arg(args,long),&buf[b],padding,sep);
						}
#endif
